...
def get_sheet_rows(channel: Optional[str] = None, unscheduled_only: bool = True, db: Session = Depends(get_db)):
    """Fetch rows from Google Sheets for the channel.
    
    By default (unscheduled_only=True), returns only rows where 'scheduled' and 'upload id' are empty.
    ...
    try:
        ...
        for r in all_rows:
            ...
            # Filter to unscheduled only if requested
